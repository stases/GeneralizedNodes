...
def train(hyperparameters):
    ...
    with wandb.init() as run:
        config = {**hyperparameters, 'node_features': 9,
                  'out_features': 1,
                  'data_dir': "./data/md17",
                  'name': "aspirin CCSD"}
...
def run_sweep_run(hyperparameters):
    wandb.init()
...
# Define a wrapper function to pass hyperparameters to the run_sweep_run function
def run_wrapper(run):
    run_sweep_run(run.config)

# Run the sweep agent with parallel processes
agent_runs = wandb.agent(sweep_id, function=run_wrapper, count=num_processes)
pool.map(agent_runs, range(num_processes))
